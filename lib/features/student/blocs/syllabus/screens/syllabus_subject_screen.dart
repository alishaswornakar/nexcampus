...
/// Shows the courses a teacher has actually created for the student's
/// chosen department + tapped semester (same live source as
/// Courses/Notes).
///
/// Dropdown per course ("Drive" / "Teacher Material"):
///  - "Drive" opens the Syllabus Drive-folder link, resolved via
///    `_subjectDataFor` + `getSyllabusLink` (`semester_subjects.dart`).
///    This is the fix for the `rootDriveFolder`-on-everything bug —
///    the old screen imported a stale `semester_data1.dart` copy with
///    no `syllabusDrive` links populated.
///  - "Teacher Material" streams the real notes/material the teacher
///    uploaded for this course, matched by `courseId`.
...
class _SyllabusSubjectScreenState extends State<SyllabusSubjectScreen> {
  ...
  Future<void> _loadDepartment() async {
    try {
      ...
      if (user == null) {
        setState(() {
          _departmentError = 'You need to be signed in to view the syllabus.';
          _loadingDepartment = false;
        });
        return;
      }
...
/// Lists the real files a teacher has uploaded for [course], matched
/// by `courseId` — same connection used on the Courses and Notes tabs.
...
/// Opens a single [NoteModel]'s uploaded file, picking the right
/// viewer by extension (pdf / image / doc-ppt-via-webview).
