...
/// Department + semester picker for the student's Notes tab.
///
/// Mirrors `CoursesScreen`'s structure and department list exactly, so
/// the Notes tab feels and behaves the same way as the Courses tab.
/// Selecting a semester pushes [NotesSubjectScreen], which streams the
/// department's real Firestore courses for that semester and gives
/// each course a "Drive" / "Teacher Material" dropdown — same pattern
/// as the Courses tab.
...
class _NotesScreenState extends State<NotesScreen> {
  // Same department -> semester-count map used on the Courses tab and
  // on the teacher side (department_semester_selection_screen.dart),
  // so all three flows stay in sync.
          ...
          child: Column(
            ...
            children: [
              ...
              if (_selectedDepartment != null) ...[
                ...
                const SizedBox(height: 6),
                const Text(
                  'Select a semester to view its Notes.',
                  style: TextStyle(fontSize: 13, color: AppTheme.textSecondary),
                ),
                const SizedBox(height: 20),
                GridView.builder(
