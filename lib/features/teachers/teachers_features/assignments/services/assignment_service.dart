...
class AssignmentService {
  ...
  /// Update Assignment
  Future<void> updateAssignment({required AssignmentModel assignment}) async {
    try {
      await assignmentCollection.doc(assignment.id).update(assignment.toMap());
    } catch (e) {
  ...
  Future<AssignmentModel?> getAssignment(String assignmentId) async {
    final doc = await assignmentCollection.doc(assignmentId).get();
  ...
  Future<void> updateSubmissionCount({
    ...
    await assignmentCollection.doc(assignmentId).update({
